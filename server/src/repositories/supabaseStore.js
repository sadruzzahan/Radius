...
export class SupabaseStore {
  ...
  async createMessage(data) {
    const { data: row, error } = await this.client
      .from("chat_messages")
      .insert([{ listing_id: data.listingId, sender_id: data.senderId, recipient_id: data.recipientId, body: data.body }])
      .select("*")
      .single();
    throwIfError(error);
  ...
  async listMessages(listingId) {
    ...
    throwIfError(error);
    return data.map((row) => ({ id: row.id, listingId: row.listing_id, senderId: row.sender_id, recipientId: row.recipient_id, body: row.body, createdAt: row.created_at }));
  }

  async createReview(data) {
    const { data: row, error } = await this.client
      .from("reviews")
      .insert([{ trade_id: data.tradeId, listing_id: data.listingId, reviewer_id: data.reviewerId, reviewee_id: data.revieweeId, rating: data.rating, comment: data.comment ?? "" }])
      .select("*")
      .single();
    throwIfError(error);
