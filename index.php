...
$adminReviewedCount=count(array_filter($items,fn($x)=>is_admin_reviewed((string)($x['trust_status']??''),(string)($x['status']??''))));
...
<section class="trust-hero">
  <div class="trust-hero-copy">
    ...
    <div class="trust-radar-stats"><span><strong><?=count($items)?></strong> visible listings</span><span><strong><?=$adminReviewedCount?></strong> admin-reviewed</span><span><strong><?=$verifiedSellers?></strong> verified sellers</span></div>
  </div>
  <div class="trust-radar-shell" data-radar>
    <div class="trust-radar-panel">
      ...
      <?php $n=max(count($radarItems),1);foreach($radarItems as $i=>$l):$angle=($i/$n)*M_PI*2-M_PI/2;$distance=(float)($l['distance_km']??2.2);$radius=20+min($distance/5,1)*30;$x=50+$radius*cos($angle);$y=50+$radius*sin($angle);$state=trust_state((string)($l['trust_status']??''),(string)$l['status']);?>
      <button type="button" class="trust-radar-node <?=$state?><?=$i===0?' selected':''?>" style="left:<?=number_format($x,2,'.','')?>%;top:<?=number_format($y,2,'.','')?>%" data-radar-node data-id="<?=$l['id']?>" data-title="<?=e($l['title'])?>" data-price="<?=e(money($l['price']))?>" data-distance="<?=number_format($distance,1)?> km" data-state="<?=$state?>" data-trust="<?=e(trust_label((string)($l['trust_status']??''),(string)$l['status']))?>" data-image="<?=e(listing_visual($l))?>" data-href="/listing.php?id=<?=$l['id']?>"><img src="<?=e(listing_visual($l))?>" alt=""><span class="trust-radar-node-distance"><?=number_format($distance,1)?>km</span></button>
      <?php endforeach;?>
      <?php if($radarItems):$first=$radarItems[0];?><div class="trust-radar-card <?=trust_state((string)($first['trust_status']??''),(string)$first['status'])?>" data-radar-card><img src="<?=e(listing_visual($first))?>" alt="" data-radar-card-image><div class="trust-radar-card-body"><strong data-radar-card-title><?=e($first['title'])?></strong><span class="trust-radar-price" data-radar-card-price><?=money($first['price'])?></span><span data-radar-card-distance><?=number_format((float)($first['distance_km']??0),1)?> km away</span><span data-radar-card-trust><?=e(trust_label((string)($first['trust_status']??''),(string)$first['status']))?></span><a class="btn btn-primary btn-sm" data-radar-card-link href="/listing.php?id=<?=$first['id']?>">View Listing</a></div></div><?php endif;?>
    </div>
  </div>
</section>

<div class="section-head"><div><h2>Nearby listings</h2><p>Only approved listings appear here. Flagged or unchecked posts stay in the admin moderation queue.</p></div><a href="/listings.php">View all →</a></div>
...
<?php foreach(array_slice($items,0,12) as $l):?><a class="listing-card" href="/listing.php?id=<?=$l['id']?>"><div class="card-image-wrap"><img class="card-image" src="<?=e(listing_visual($l))?>" alt="<?=e($l['title'])?>"></div><div class="card-content"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px"><h3 class="card-title"><?=e($l['title'])?></h3><div class="card-price"><?=money($l['price'])?></div></div><p class="card-desc"><?=e($l['description'])?></p><div class="card-meta"><span class="meta-pill"><?=e($l['category'])?></span><span class="meta-pill"><?=e($l['item_condition'])?></span><span><?=number_format((float)($l['distance_km']??0),1)?>km</span><span class="badge <?=e(trust_class((string)($l['trust_status']??''),(string)$l['status']))?>"><?=e(trust_label((string)($l['trust_status']??''),(string)$l['status']))?></span></div></div></a><?php endforeach;?>
</div>
</div>
